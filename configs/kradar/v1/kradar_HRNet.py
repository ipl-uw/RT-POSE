...
model = dict(
    ...
    neck=dict(
        ...
        ds_num_filters=[90, 180], # num of output channels for each block
        us_layer_strides=[1, 2], # upstrides of blocks
        us_num_filters=[180, 180], # num of output channels for each deblock
        num_input_features=720, # 
        logger=logging.getLogger("RPN"),
    ),
    bbox_head=dict(
        type="CenterHead",
        in_channels=sum([180, 180]), # sum of us_num_filters
        tasks=tasks,
        dataset='kradar',
        weight=0.25,
...
DATASET = dict(
  TYPE_LOADING='path',
  DIR=dict(
    LIST_DIR=['/mnt/nas_kradar/kradar_dataset/dir_tmp']
  ),
  PATH_SPLIT={
