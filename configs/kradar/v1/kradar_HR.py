...
model = dict(
    ...
    neck=dict(
        ...
        ds_num_filters=[8, 16], # num of output channels for each block
        us_layer_strides=[1, 2], # upstrides of blocks
        us_num_filters=[16, 16], # num of output channels for each deblock
        num_input_features=16, 
        logger=logging.getLogger("RPN"),
    ),
    bbox_head=dict(
        type="CenterHead",
        in_channels=sum([16, 16]), # sum of us_num_filters
        tasks=tasks,
        dataset='kradar',
        weight=0.25,
